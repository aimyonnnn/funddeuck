...
<jsp:include page="../common/admin_top.jsp"/>
	...
	<div class="d-flex flex-row justify-content-center my-5">
		<!-- form 태그 시작 -->
		<form action="adminFundingDoctor" class="d-flex flex-row justify-content-end">
			<!-- 셀렉트 박스 -->
		...
		<div class="d-flex justify-content-center">
			...
			<table class="table">
				<tr>
					...
					<th class="text-center" style="width: 7%;">프로젝트 이름</th>
					<th class="text-center" style="width: 5%;">대표자</th>
					<th class="text-center" style="width: 5%;">목표금액</th>
					<th class="text-center" style="width: 7%;">기간</th>
				...
				<c:forEach var="pList" items="${projectList}">
					<tr>
						...
						<td class="text-center">
							<a href="adminFundingDoctorDetail?project_idx=${pList.project_idx}&pageNum=${pageNum}" style="text-decoration: none; color: black;">
						...
						<c:choose>
							<c:when test="${pList.project_status eq 7 }">
								<button class="btn btn-outline-danger btn-sm" 
								onclick="location.href='adminFundingDoctorDetail?project_idx=${pList.project_idx}&pageNum=${pageNum}'">보기</button>
							</c:when>
							<c:when test="${pList.project_status eq 8 }">
								<button class="btn btn-outline-primary btn-sm" 
								onclick="location.href='adminFundingDoctorDetail?project_idx=${pList.project_idx}&pageNum=${pageNum}'">완료</button>
							</c:when>
						</c:choose>
						</td>
					</tr>
	                ...
	                <c:when test="${pageNum > 1}">
	                	<c:choose>
	                		<c:when test="${not empty param.searchType and not empty param.searchKeyword}">
			                    <li class="page-item">
			                        <a class="page-link" aria-label="Previous" href="adminFundingDoctor?pageNum=${pageNum - 1}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">
	                		...
	                		<c:otherwise>
			                    <li class="page-item">
			                        <a class="page-link" aria-label="Previous" href="adminFundingDoctor?pageNum=${pageNum - 1}">
			                            <span aria-hidden="true">&laquo;</span>
			                        </a>
			                    </li>
	                		</c:otherwise>
	            ...
	            <c:forEach var="i" begin="${pageInfo.startPage}" end="${pageInfo.endPage}">
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                        ...
	                        <c:choose>
	                            <c:when test="${not empty param.searchType and not empty param.searchKeyword}">
	                                <li class="page-item"><a class="page-link" href="adminFundingDoctor?pageNum=${i}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">${i}</a></li>
	                            </c:when>
	                            <c:otherwise>
	                                <li class="page-item"><a class="page-link" href="adminFundingDoctor?pageNum=${i}">${i}</a></li>
	                            </c:otherwise>
	                        </c:choose>
	                    </c:otherwise>
	                </c:choose>
	            ...
	            <c:choose>
	                ...
	                <c:when test="${pageNum < pageInfo.maxPage}">
	                	<c:choose>
	                		<c:when test="${not empty param.searchType and not empty param.searchKeyword}">
			                    <li class="page-item">
			                        <a class="page-link" aria-label="Next" href="adminFundingDoctor?pageNum=${pageNum + 1}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">
	                		...
	                		<c:otherwise>
			                    <li class="page-item">
			                        <a class="page-link" aria-label="Next" href="adminFundingDoctor?pageNum=${pageNum + 1}">
			                            <span aria-hidden="true">&raquo;</span>
			                        </a>
			                    </li>
	                		</c:otherwise>
